...
- **Karpathy principles in every agent** — The 4 principles (Think Before Coding, Simplicity First, Surgical Changes, Goal-Driven Execution) are now embedded in all templates generated by `octechpus init`. Every agent in your pipeline follows them automatically:
  - **Architect** declares assumptions and defines testable success criteria before planning
  - **Coder** applies Simplicity + Surgical constraints with a pre-delivery self-checklist
  - **Reviewer** enforces K1-K4 as BLOCKER validations (not suggestions)
  - **Maestro** converts vague demands into measurable outcomes before routing
  - **CLAUDE.md** carries all 4 principles as visible universal rules for every contributor
